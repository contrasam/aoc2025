...
def rotate(rotation: String, currentPos: Int): Int =
  ...
  if isLeft then ceilRotation(currentPos - rotationAmount)
  else ceilRotation(currentPos + rotationAmount)
...
def rotateV2(rotation: String, currentPos: Int): (Int, Int) =
  ...
  val nextPos =
    if isLeft then ceilRotation(currentPos - clicks)
    else ceilRotation(currentPos + clicks)

  val crossesZero =
    if isLeft then currentPos != 0 && currentPos <= remainingClicks
    else currentPos + remainingClicks > 99
