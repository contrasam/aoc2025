...
@main def day02(): Unit =
  ...
  val invalidProductIds =
    ranges.map(parseProductIdRange).flatMap(getInvalidProducIdsInRange)
  val invalidProductIdsV2 =
    ranges.map(parseProductIdRange).flatMap(getInvalidProducIdsInRangeV2)
